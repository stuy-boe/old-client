...
export const Vote: React.FC = props => {
	...
	const startTimestring = moment(election.startTime).tz('America/New_York').format('LLLL z');

	return (
		<div>
			<Helmet>
				<title>Vote: {election.name} | Stuy BOE Voting Site</title>
				<meta property="og:title" content={`Vote: ${election.name} | Stuy BOE Voting Site`}/>
				<meta property="og:description" content={`Vote for ${election.name}. Starts ${startTimestring}.`}/>
			</Helmet>
